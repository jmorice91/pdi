...
int main(int argc, char** argv)
{
	...
	if (is_client) {
		...
		PDI_multi_expose("damaris_parameter_get", "prm_name", "DEPTH", PDI_OUT, "prm_buffer", &DEPTH, PDI_INOUT, "prm_size", &int_size, PDI_OUT, NULL);
		PDI_multi_expose(
			"damaris_parameter_get",
			"prm_name",
			"whd_layout",
		...
		if (verbose > 0) fprintf(stdout, "C++ Input paramaters found: v=%d r=%d (0 is not found)\n", verbose, rank_only);
		...
		PDI_multi_expose("damaris_set_position", "pos_var_name", "cube_i", PDI_OUT, "position_cube", position_cube, PDI_OUT, NULL);
		PDI_multi_expose("damaris_set_position", "pos_var_name", "cube_f", PDI_OUT, "position_cube", position_cube, PDI_OUT, NULL);
		...
		for (i = 0; i < MAX_CYCLES; i++) {
			...
			if (verbose == 0) { // default - do not print values to screen
				...
				if (rank_client == 0) printf("C++    iteration  %d , Sum () = %ld\n", i, reduction_result);
				...
				PDI_multi_expose("damaris_write", "w_var_name", "array_sum", PDI_OUT, "array_sum", &array_sum, PDI_OUT, NULL);
			}

			//damaris_write("cube_i" , cube);
			//damaris_write("cube_f" , cube_f);
			PDI_multi_expose("damaris_write", "w_var_name", "cube_i", PDI_OUT, "cube", cube, PDI_OUT, NULL);
			PDI_multi_expose("damaris_write", "w_var_name", "cube_f", PDI_OUT, "cube_f", cube_f, PDI_OUT, NULL);
			...
			PDI_multi_expose("damaris_write", "w_var_name", "last_iter", PDI_OUT, "MAX_CYCLES", &MAX_CYCLES, PDI_OUT, NULL);
			sleep(time);
			...
			if (rank_client == 0) {
				if (verbose > 0) printf("C++    iteration %d done in %f seconds\n", i, (t2 - t1));
			}
			fflush(stdin);
